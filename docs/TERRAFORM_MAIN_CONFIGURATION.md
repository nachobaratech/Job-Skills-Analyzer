...
This document contains the complete Terraform `main.tf` file that manages all 22 AWS resources for the Job Skills Analyzer project.

## Purpose

This file demonstrates Infrastructure as Code (IaC) implementation, showing how all AWS resources can be defined, versioned, and deployed through code rather than manual console configuration.

## Resources Managed (22 total)

1. **S3 Buckets (3)**: Raw data, curated data, Athena results
2. **S3 Public Access Blocks (3)**: Security controls for all buckets
3. **Lambda Function (1)**: ETL trigger function
4. **Lambda Permission (1)**: S3 invoke permission
5. **CloudWatch Log Group (1)**: Lambda execution logs
6. **CloudWatch Dashboard (1)**: System monitoring
7. **CloudWatch Metric Alarm (1)**: Error rate monitoring
8. **CloudWatch Event Rule (1)**: Daily scheduling
9. **SNS Topic (1)**: Alert notifications
10. **SNS Subscription (1)**: Email notifications
11. **S3 Bucket Notification (1)**: Lambda trigger
12. **Null Resource (1)**: Athena table creation

## Complete main.tf Content
```hcl
# ============================================================================
...
The configuration correctly handles dependencies between resources:
- S3 buckets must exist before bucket policies
- Lambda must exist before S3 notifications
- SNS topic must exist before Lambda environment variables reference it

### 2. Dynamic References
Uses Terraform interpolation for dynamic values:
```hcl
environment {
  variables = {
    SNS_TOPIC_ARN = aws_sns_topic.alerts.arn
...
Terraform tracks all managed resources in `terraform.tfstate`. This file is critical for:
- Detecting infrastructure drift
- Planning updates
- Safely destroying resources

**Note:** In production, state should be stored remotely (e.g., S3 with DynamoDB locking).
